...
- The specification also provided the following criteria:
    ...
    - Each restauranti's data contains many pieces of information and the only information that we are required to display are:
        - Name
        - Cuisines
        - Rating (as a number)
        - Address 
...
- The 'RestaurantSearch' page contains conditional render that displays the error message only if the useState value 'displayError' is not empty.
- This message is displayed for 2 seconds to the user using a timeout function.

This approach keeps error handling simple appropriate for an application of this size.
...
- Testing was carried out using Vitest, which is a testing framework that is recommended for use in TypeScript applications.
...
- Tests were carried to test for behaviour and also edge cases. These include: 
    ...
    - Variations in upper and lower case.
